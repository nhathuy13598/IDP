...
public:
	...
	*/
	...
	int Convert(IplImage* sourceImage, IplImage* destinationImage, int type);
 	...
 	//-------------------------------HÀM CHUYỂN ĐỔI BỔ SUNG ĐỂ HỖ TRỢ HISTOGRAM EQUALIZATION------------------------------------
	// Chỉ tìm thông số của các thành phần trong mô hình màu mới (trên 1 pixel) 

	/* Đổi từ hệ RGB sang HSI 
	Từ r, g, b tìm h, s, i*/
	void RGB2HSI(float r, float g, float b, float &h, float &s, float &i);

	/* Đổi từ hệ HSI sang RGB
	Từ h, s, i tìm r, g, b*/
